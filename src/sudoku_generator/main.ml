...
let sudoku_variables n = List.concat (generate_variables (n*n))

let generate_value_equations n =
  List.map (fun (i,j) -> generate_equation n i j)
    (generate_indices (n*n))

let sudoku_variables_2 = sudoku_variables 2
let sudoku_variables_3 = sudoku_variables 3
...
(* let toto = generate_box 3 2 2 *)

let build_sudoku_state n : state =
  let init_state = {
      fresh_index = 0;
      public_variables = sudoku_variables n;
      private_variables = [];
      equations = [];
      private_definitions = []
    } in
...
let toto = build_sudoku_state 2

let _ = oc_state toto "toto"
